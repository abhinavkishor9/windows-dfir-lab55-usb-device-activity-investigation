# Troubleshooting Notes — Lab 55 USB Device Activity Investigation
...
|---|---|
| `USBSTOR` unavailable | Investigated alternative USB artifacts |
| USB Registry branch available | Investigated `Enum\USB` |
| USB device identifiers returned | Treated as USB enumeration evidence |
| DeviceClasses available | Used as supporting device-interface evidence |
| System provider search returned no results | Documented queried-window limitation |
| Broad USB search returned no results | Documented queried-window limitation |
| Current disk was NVMe | Recorded current storage state |
| No current USB disk identified | Did not infer historical absence |
| 4624 available | Used for session context |
| 4624 example was Logon Type 5 | Treated as service logon |
| Sysmon Event ID 1 available | Used for process context |
| Sysmon Event ID 3 available | Used for network context |
| Filesystem activity available | Used as supporting evidence |
| No USB transfer simulated | Preserved artifact-validation objective |

---

# Key Troubleshooting Lessons

## 1. Missing Artifacts Are Findings

If an expected artifact is unavailable, document it.

Do not manufacture it.

---

## 2. Search Scope Matters

A search of the last 500 or 1000 events does not represent the entire history of the machine.

Always document the search scope.

---

## 3. Current State Is Not Historical State

A current NVMe disk does not prove that a USB device was never connected.

Likewise, the absence of a currently connected USB device does not establish historical absence.

---

## 4. Artifact Presence Is Not Activity Proof

A USB enumeration entry demonstrates that Windows maintains information about a USB-related device.
...
## 5. User Attribution Requires Correlation

A successful Windows logon event should not automatically be associated with USB activity.
...
## 6. Network Activity Is Supporting Evidence

Sysmon Event ID 3 can add context.

It cannot independently prove USB data theft.

---

## 7. Preserve Existing Evidence

Do not:

- Clear event logs.
- Delete Registry artifacts.
- Modify device enumeration.
- Disable Sysmon.
- Disable security tools.

The goal is investigation, not evidence destruction.

---

# Final Lesson

The central troubleshooting lesson from Lab 55 is:

> **A missing USB artifact does not mean the investigation has failed.**

A strong DFIR analyst identifies what artifacts are actually available, validates their meaning, documents limitations, and avoids claiming more than the evidence supports.

The goal is not to prove USB activity at any cost.

The goal is to determine what the available evidence can reliably establish.
